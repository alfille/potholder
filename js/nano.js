...
var require_package = __commonJS({
  "node_modules/nano/package.json"(exports2, module2) {
    module2.exports = {
      name: "nano",
...
var require_changesreader = __commonJS({
  "node_modules/nano/lib/changesreader.js"(exports2, module2) {
    var EventEmitter = require("events").EventEmitter;
    var AbortController = global.AbortController || require_node_abort_controller().AbortController;
    var stream2 = require("stream");
    var EVENT_BATCH = "batch";
    var EVENT_CHANGE = "change";
    var EVENT_SEQ = "seq";
    var EVENT_ERROR = "error";
    var liner = () => {
      const liner2 = new stream2.Transform({ objectMode: true });
      liner2._transform = function(chunk, encoding, done) {
    ...
    var changeProcessor = (ee, batchSize) => {
      const changeProcessor2 = new stream2.Transform({ objectMode: true });
      const buffer = [];
      changeProcessor2.lastSeq = "0";
      changeProcessor2._transform = function(chunk, encoding, done) {
        ...
        try {
          ...
          if (match) {
            changeProcessor2.lastSeq = JSON.parse(match[1]);
          }
          done();
        }
      };
      changeProcessor2._flush = function(done) {
        if (buffer.length > 0) {
          ee.emit(EVENT_BATCH, buffer.splice(0, buffer.length));
        }
        done();
      };
      return changeProcessor2;
    };
    var ChangesReader2 = class {
      // constructor
      ...
      start(opts) {
        ...
        self2.started = true;
        opts = opts || {};
        Object.assign(self2, opts);
        let delay = 0;
        const work = async () => {
          do {
            if (!self2.paused) {
              self2.abortController = new AbortController();
              const req = {
                method: "post",
                db: self2.db,
                path: "_changes",
                signal: self2.abortController.signal,
                qs: {
                  feed: "longpoll",
                  timeout: self2.timeout,
                  since: self2.since,
                  limit: self2.batchSize,
                  include_docs: self2.includeDocs
                },
                body: {}
              };
              if (self2.fastChanges) {
                req.qs.seq_interval = self2.batchSize;
              }
              if (self2.selector) {
                req.qs.filter = "_selector";
                req.body.selector = self2.selector;
              }
              Object.assign(req.qs, opts.qs);
              try {
                const data = await self2.request(req);
                self2.abortController = null;
                delay = 0;
                if (data && data.last_seq && data.last_seq !== self2.since) {
                  self2.since = data.last_seq;
                  self2.ee.emit(EVENT_SEQ, self2.since);
                }
                if (self2.stopOnEmptyChanges && data && typeof data.results !== "undefined" && data.results.length === 0) {
                  self2.continue = false;
                }
                if (data && data.results && data.results.length > 0) {
                  for (const i in data.results) {
                    self2.ee.emit(EVENT_CHANGE, data.results[i]);
                  }
                  if (self2.wait) {
                    self2.pause();
                  }
                  self2.ee.emit(EVENT_BATCH, data.results);
                }
              } catch (err) {
                if (err && err.statusCode && err.statusCode >= 400 && err.statusCode !== 429 && err.statusCode < 500) {
                  self2.continue = false;
                } else {
                  delay = delay ? Math.min(6e4, delay * 2) : 5e3;
                }
                self2.ee.emit(EVENT_ERROR, err);
              }
            }
            if (self2.paused && delay === 0) {
              delay = 100;
            }
            if (self2.continue && delay > 0) {
              await self2.sleep(delay);
            }
          } while (self2.continue);
          self2.ee.emit("end", self2.since);
          self2.setDefaults();
        };
        work();
        return self2.ee;
      ...
      spool(opts) {
        ...
        const lin = liner();
        const cp = changeProcessor(self2.ee, self2.batchSize);
        self2.request(req).pipe(lin).pipe(cp).on("finish", (lastSeq) => {
          setTimeout(() => {
            self2.ee.emit("end", cp.lastSeq);
          }, 10);
        }).on(EVENT_ERROR, (e) => {
          self2.ee.emit(EVENT_ERROR, e);
        });
        return self2.ee;
      }
    };
    module2.exports = ChangesReader2;
  }
});

// node_modules/nano/lib/cookie.js
var require_cookie = __commonJS({
  "node_modules/nano/lib/cookie.js"(exports2, module2) {
    ...
    var CookieJar2 = class {
      ...
      findByName(url, name) {
        ...
        for (let i = 0; i < this.jar.length; i++) {
          const c = this.jar[i];
          if (c.origin === parsedURL.origin && c.name === name && c.ts >= now) {
            return i;
          }
        }
        return -1;
      ...
      getCookieString(url) {
        ...
        for (i = 0; i < this.jar.length; i++) {
          const c = this.jar[i];
          if ((c.origin === parsedURL.origin || c.domain && parsedURL.hostname.endsWith(c.domain)) && c.ts >= now) {
...
var require_multipart = __commonJS({
  "node_modules/nano/lib/multipart.js"(exports2, module2) {
    var CRLF = "\r\n";
    var DASHES = "--";
    var MultiPartFactory2 = class {
      // constructor
      constructor(parts) {
    ...
    module2.exports = MultiPartFactory2;
  }
});
...
var AGENT_DEFAULTS = { keepAlive: true, maxSockets: 50, keepAliveMsecs: 3e4 };
var defaultHttpAgent = new http.Agent(AGENT_DEFAULTS);
var defaultHttpsAgent = new https.Agent(AGENT_DEFAULTS);
var SCRUBBED_STR = "XXXXXX";
var ChangesReader = require_changesreader();
var CookieJar = require_cookie();
var MultiPartFactory = require_multipart();
...
module.exports = exports = function dbScope(cfg) {
  ...
  function scrubRequest(req, cloned) {
    ...
    if (req.auth) {
      if (!cloned) {
        req.auth = JSON.parse(JSON.stringify(req.auth));
      }
  ...
  const streamResponseHandler = function(response, req, stream2) {
    const statusCode = response.status || response.response && response.response.status || 500;
    if (response.isAxiosError && response.response) {
      response = response.response;
    }
  ...
  function _serializeAsUrl(db2) {
    if (typeof db2 === "object" && db2.config && db2.config.url && db2.config.db) {
      return urlResolveFix(db2.config.url, encodeURIComponent(db2.config.db));
    } else {
      try {
        const parsed = new URL2(db2);
        return parsed.toString();
      } catch (e) {
        return urlResolveFix(cfg.url, encodeURIComponent(db2));
      }
    }
  }
...
function urlResolveFix(couchUrl, dbName) {
  ...
  return new URL2(dbName, couchUrl).toString();
}
